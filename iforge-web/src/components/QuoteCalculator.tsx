...
export default function QuoteCalculator() {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white border border-neutral-200 rounded-3xl p-6 sm:p-10 shadow-lg shadow-black/5">
      {/* Progress Header */}
      <div className="flex items-center justify-between pb-6 border-b border-neutral-100 mb-8">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#FDE8EA] text-[#E61E32] flex items-center justify-center">
            <Calculator className="w-5 h-5" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-black">Project Estimator Wizard</h2>
            <p className="text-xs text-neutral-500 font-medium">Step {step} of 3</p>
          </div>
        </div>
      ...
      {!submitted ? (
        <div>
          ...
          {step === 1 && (
            <div className="space-y-6 animate-in fade-in duration-200">
              <div>
                <h3 className="text-lg font-bold text-black">Select Your Primary IT Service</h3>
                  ...
                  return (
                    <button
                      ...
                      className={`text-left p-4 rounded-2xl border transition-all cursor-pointer flex items-start gap-3.5 ${
                        isSelected
                          ? "bg-[#FDE8EA]/60 border-[#E61E32] shadow-xs"
                          : "bg-white border-neutral-200 hover:border-neutral-300 hover:bg-neutral-50/50"
                      }`}
                    >
                      <div className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 mt-0.5 ${
                        isSelected ? "border-[#E61E32] bg-[#E61E32] text-white" : "border-neutral-300"
                      }`}>
                        {isSelected && <Check className="w-3 h-3 stroke-[3]" />}
                      </div>
                      <div>
                        <div className={`text-sm font-bold ${isSelected ? "text-black" : "text-neutral-800"}`}>
                          {s.title}
                        </div>
          ...
          {step === 2 && (
            <div className="space-y-6 animate-in fade-in duration-200">
              <div>
                <h3 className="text-lg font-bold text-black">Project Scale & Delivery Horizon</h3>
                <p className="text-xs sm:text-sm text-neutral-600 mt-1">
              ...
              <div className="pt-4 flex items-center justify-between border-t border-neutral-100">
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-semibold text-neutral-600 hover:bg-neutral-100 transition-colors cursor-pointer"
                >
          ...
          {step === 3 && (
            <form onSubmit={handleSubmit} className="space-y-6 animate-in fade-in duration-200">
              <div>
                <h3 className="text-lg font-bold text-black">Scope Summary & Dispatch</h3>
                <p className="text-xs sm:text-sm text-neutral-600 mt-1">
              ...
              <div className="p-5 rounded-2xl bg-neutral-50 border border-neutral-200 space-y-2">
                <div className="flex items-center justify-between text-xs pb-2 border-b border-neutral-200">
                  <span className="text-neutral-500 font-medium">Selected Discipline:</span>
                  <span className="font-bold text-black">{currentServiceObj.title}</span>
                </div>
                <div className="flex items-center justify-between text-xs pb-2 border-b border-neutral-200">
                  <span className="text-neutral-500 font-medium">Target Scope:</span>
                  <span className="font-bold text-[#E61E32]">{currentScopeObj.title} ({currentScopeObj.budget})</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-neutral-500 font-medium">Delivery Timeline:</span>
                  <span className="font-bold text-black">{timelineOptions.find(t => t.id === timeline)?.title}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-neutral-50 border border-neutral-200 rounded-xl px-4 py-2.5 text-sm text-black focus:outline-none focus:border-[#E61E32] focus:bg-white transition-all"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-black uppercase tracking-wider mb-1.5">
        ...
        <div className="text-center py-8 space-y-4 animate-in fade-in duration-300">
          ...
          <p className="text-sm text-neutral-600 max-w-md mx-auto leading-relaxed">
            Thank you, <span className="font-bold text-black">{clientInfo.name}</span>. We have generated preliminary architecture parameters for your <strong>{currentServiceObj.title}</strong> initiative. A Principal Consultant will email your structured proposal within 2 hours.
          </p>
          <div className="pt-4">
            <button
              type="button"
              ...
              className="px-5 py-2.5 rounded-xl text-xs font-semibold text-black bg-neutral-100 hover:bg-neutral-200 transition-colors cursor-pointer"
            >
